...
const obj1Path = './search_callMe_latest_date.json';  //new json from shop_data [callMe.json]
const obj2Path = './search_callMe_earlier_date.json'; //old json [all_callMeFromShop_data.json]
...
for (const key of keysToCompare) {
  ...
  if (obj1Callers !== obj2Callers) {
    const addNum = obj1Callers - obj2Callers;
    differences.callersChange[key] = {
      有變動: `診間數量從 '${obj2Callers}間' 變成 '${obj1Callers}間', 需印出A4Qrcode ${addNum}張` //物件文字顯示法
    };
...
console.log(`此次刪除掉的診間有: ${Object.keys(differences.deletedStore).length}間`);
...
const missingInObj1JsonPath = path.join(differentJsonFileDir, '刪除掉的店家.json');
const missingInObj2JsonPath = path.join(differentJsonFileDir, '多出來的店家.json');

fs.writeFileSync(callersDifferenceJsonPath, JSON.stringify(differences2File.診間數量變動, null, 2));
fs.writeFileSync(missingInObj1JsonPath, JSON.stringify(differences2File.刪除掉的店家, null, 2));
fs.writeFileSync(missingInObj2JsonPath, JSON.stringify(differences2File.多出來的店家, null, 2));

console.log('相關物件檔案, 請到\'DifferentJsonFile\'資料夾內搜尋\n');
