...
'''
send_telegram_notification 發送時機：
連線恢復時：斷線超過 5 秒後恢復連線

連線異常時：按照設定的時間間隔發送通知 
should_send_notification & "notification_intervals": [5, 30, 60, 300, 600, 1800, 3600]
斷線 30 秒後發第一次通知
斷線 60 秒後發第二次通知
斷線 5 分鐘後發第三次通知
斷線 10 分鐘後發第四次通知
...
斷線 60 分鐘後發第六次通知
之後每 60 分鐘發送一次通知
這樣設計的目的是在連線異常時，初期較密集通知，後期則降低通知頻率，避免過度打擾。
'''
...
class WebSocketMonitor:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.last_quality_check = time.time() - 60 + 5    # 0
        ...
        # for perf_counter timing if using builtin ping
        self._last_builtin_ping_perf: Optional[float] = None
        ...
        # 區分手動關閉（例如收到 SIGINT） vs 程式內部為了重連呼叫的關閉
        self._manual_close = False

    # ---------- console + logging helper ----------
    def _log_console(self, message: str, level: str = "info", emoji: str = ""):
        ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        line = f"{emoji} [{ts}] {message}"
        print(line)
        if level == "info":
    ...
    async def run_monitoring(self):
        ...
        self._log_console("啟動 WebSocket 監控程式...", emoji="🟡")
        ...
        if not initial_success:
            self._log_console("初始連線失敗，開始重連流程", level="error", emoji="🔴")
            ...
            try:
                ...
                if current_time - self.last_connection_check >= 600:
                    if self.connection_state != ConnectionState.CONNECTED:
                        self._log_console("執行定期連線狀態檢查 - 目前斷線", emoji="🟡")
                ...
                # 連線品質檢查（每 quality_check_interval 秒）
                if (self.connection_state == ConnectionState.CONNECTED and
                    ...
                    try:
                        ...
                        if quality_result.get("quality_ok"):
                            # 品質良好，重置失敗計數
                            self.quality_check_failures = 0
                        else:
                            # 品質檢查失敗
                            ...
                            logging.warning(
                                f"品質檢查失敗 ({self.quality_check_failures}/{self.max_quality_failures}): {quality_result}")

                            # 連續失敗達到閾值，強制重連
                            if self.quality_check_failures >= self.max_quality_failures:
                                self._log_console(
                                    f"品質檢查連續失敗 {self.quality_check_failures} 次，強制重新連線", level="error", emoji="🔴")
                        ...
                        self.log_check_result("連線品質", quality_result.get(
                            "quality_ok", False), quality_result)
                        self.last_quality_check = current_time

                    except Exception as e:
                        self.quality_check_failures += 1
                        logging.error(
                            f"品質檢查過程中發生錯誤 ({self.quality_check_failures}/{self.max_quality_failures}): {e}")
                        ...
                        if self.quality_check_failures >= self.max_quality_failures:
                            self._log_console(
                                f"品質檢查異常累積達 {self.quality_check_failures} 次，強制重新連線", level="error", emoji="🔴")
                            self.connection_failed_since = current_time
                            self._update_connection_state(
                                ConnectionState.DISCONNECTED)
                            await self.safe_close()
                ...
                # 定期狀態報告（每 status_report_interval 秒）
                ...
                logging.error(
                    f"監控循環發生錯誤 ({consecutive_errors}/{max_consecutive_errors}): {e}")
    ...
    async def _handle_continuous_reconnect(self, current_time: float):
        """處理持續重連"""
        self._log_console(
            f"執行持續重連嘗試 (#{self.reconnect_attempts + 1})", emoji="🔴")
    ...
    def _update_connection_state(self, new_state: ConnectionState):
        ...
        if old_state != new_state:
            ...
            if new_state == ConnectionState.DISCONNECTED:
                self._log_console("檢測到斷線！立即嘗試重新連線...",
                                  level="warning", emoji="🔴")
            elif new_state == ConnectionState.CONNECTED:
                self._log_console("連線成功建立！", emoji="🟢")
    ...
    def _reset_notification_state(self):
        ...
        if self.connection_failed_since is not None:
            ...
            if downtime > self.recovery_notification_threshold:    # 斷線時間大於 self.recovery_notification_threshold 秒
                ...
                self._log_console(f"連線恢復！中斷時間: {downtime} 秒", emoji="🟢")
    ...
    async def connect_and_login(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"WebSocket 連線失敗: {e}")
                return False
            ...
            print(f"RAW SEND: {login_msg} ", end = '')  # <--- 除錯用，正式上線可移除
            logging.info("WebSocket 連線成功並發送登入訊息")
        ...
        try:
            async for message in self.ws:
                ...
                if size > 10 * 1024 * 1024:
                    logging.warning(f"收到過大訊息: {size} bytes")
                    continue
    ...
    async def _cleanup_listener(self):
        ...
        # 若為手動關閉（例如 SIGINT），不要自動重連
        if self._manual_close:
            logging.info("手動關閉流程，不會自動重連")
            return

        # 短暫延遲後嘗試自動重連（如果不是手動關閉）
    ...
    async def _handle_pong_message(self, content: str, timestamp: float):
        """處理 PONG 回應"""
        # 如果使用 custom ping 以 time.time() 計時
        if self.ping_sent_time is not None:
            ...
            # reset ping_sent_time
            self.ping_sent_time = None
            return

        # 若沒有 ping_sent_time，則檢查 builtin ping perf 記錄
        if self._last_builtin_ping_perf is not None:
            # 這裡只記錄收到 pong 的時間（使用 perf_counter）
            # 我們無法從 content 得到精確 perf_counter timestamp，故只記 log
            logging.debug("收到 PONG（由伺服器回傳），但未紀錄對應的 ping_sent_time")
        else:
    ...
    async def _handle_json_message(self, data: Dict, timestamp: float):
        ...
        self._log_console(f"action:{action}", emoji="JSON")
        ...
        if action == "pong":
            ...
            if self.ping_sent_time is not None:
                ...
                logging.info(f"Pong 回應: {response_time:.1f}ms (平均 {avg:.1f}ms)")
                # 注意：不要在這裡清掉 ping_sent_time，由 check_connection_quality 負責
            return
    ...
    # async def check_connection_quality_xxx(self) -> Dict[str, Any]:
    #     """改進的連線品質檢查：
    #        1. 發送 JSON {"action": "ping"} （標準格式）
    #        2. 等待 {"action": "pong"} 回應
    #        3. 若無 pong，但最近有任何訊息 → 視為 OK
    #     """
    ...
    #     #     logging.debug(f"SENT PING: {ping_msg}")
    #     # except Exception as e:
    #     #     logging.error(f"發送 ping 失敗: {e}")
    #     #     return {"response_time_ms": None, "quality_ok": False, "reason": "send_ping_failed"}
    
    #     # # 等待 pong 回應（最多 8 秒）
    #     # for _ in range(80):  # 8秒 / 0.1秒 = 80次
    #     #     await asyncio.sleep(0.1)
    #     #     if (self.last_pong_time is not None and 
    #     #         self.ping_sent_time is not None and 
    #     #         self.last_pong_time > self.ping_sent_time):
    
    #     #         response_time_ms = (self.last_pong_time - self.ping_sent_time) * 1000
    #     #         self.ping_response_times.append(response_time_ms)
    #     #         if len(self.ping_response_times) > 10:
    #     #             self.ping_response_times.pop(0)
    ...
    #     #         quality_ok = response_time_ms < 2000  # 2秒內算 OK
    #     #         return {
    ...
    #             response_time = (self.last_pong_time -
    #                              self.ping_sent_time) * 1000
    ...
    def log_check_result(self, check_type: str, success: bool, details: Dict[str, Any]):
        ...
        log_entry = {
            ...
            "server": self.ws_url,
            "quality_failures": self.quality_check_failures,
            **details
        }

        json_log_file = self.config['logging'].get(
            'file', 'websocket_monitor_Cl1.log').replace('.log', '_json.log')
        try:
        ...
        if check_type == "連線品質":
            if details.get("quality_ok"):
                if details.get("response_time_ms") is not None:
                    # logging.info(
                    #     f"連線品質良好: 延遲 {details['response_time_ms']} ms (方法: {details.get('method')})")
                    pass
                else:
                    # logging.info(
                    #     f"連線品質良好: 方法 {details.get('method')} (原因: {details.get('reason', 'N/A')})")
                    pass
            else:
    ...
    async def safe_close(self, manual: bool = False):
        """安全關閉連線和任務
           manual=True 表示使用者手動關閉 (例如 SIGINT)，這會阻止 _cleanup_listener 觸發自動重連
        """
        # 標記是否為手動關閉
        ...
        logging.info("安全關閉完成")
        # 若是手動關閉，保持 _manual_close=True 直到程式結束；否則清除標記
        if not manual:
            self._manual_close = False
    ...
    async def handle_connection_failure(self):
        ...
        if self.connection_failed_since is None:
            ...
            self._log_console("連線失敗計時開始...", emoji="🔴")
        ...
        if self.should_send_notification():
            ...
            self._log_console(f"已發送 Telegram 通知 (斷線 {downtime} 秒)", emoji="📢")
            logging.warning(f"發送連線異常通知，下次通知索引: {self.next_notification_index}")

    async def attempt_reconnect(self) -> bool:
        """嘗試重新連線，使用簡單延遲（保留擴充為指數退避的可能）"""
        if self.reconnect_attempts >= self.max_reconnect_attempts:
            logging.error(f"達到最大重連嘗試次數 {self.max_reconnect_attempts}，停止重連")
            return False

        if self.reconnect_attempts > 0:
            delay = min(1.0 * (self.reconnect_attempts),
                        self.max_reconnect_delay)
            self._log_console(
                f"重連嘗試 #{self.reconnect_attempts}, 等待 {delay:.1f} 秒", emoji="🟡")
            logging.info(f"重連嘗試 #{self.reconnect_attempts}, 等待 {delay:.1f} 秒")
        ...
        try:
            ...
            if success:
                self._log_console(
                    f"重連成功！嘗試次數: {self.reconnect_attempts + 1}", emoji="🟢")
                ...
                self.reconnect_attempts += 1
                self._log_console(
                    f"重連失敗，當前嘗試次數: {self.reconnect_attempts}", emoji="🔴")
    ...
    def _report_current_status(self):
        ...
        print(
            f"   品質失敗: {self.quality_check_failures}/{self.max_quality_failures}")
        if self.connection_failed_since is not None:
            downtime = int(time.time() - self.connection_failed_since)
...
def setup_global_logging(config: Optional[Dict[str, Any]] = None):
    ...
    if config and 'logging' in config:
        log_file = config['logging'].get('file', 'websocket_monitor_cl1.log')
        level_name = config['logging'].get('level', 'INFO')
    else:
        log_file = os.getenv('LOG_FILE', 'websocket_monitor_cL1.log')
...
CONFIG = {
    ...
    "logging": {
        "file": os.getenv('LOG_FILE', 'websocket_monitor_CL1.log'),
...
async def main():
    ...
    def signal_handler(signum, frame):
        ...
        # 手動關閉，避免 _cleanup_listener 自動重連
        asyncio.create_task(monitor.safe_close(manual=True))
    ...
    try:
        ...
        await monitor.safe_close(manual=True)
        logging.info("程式正常退出")
