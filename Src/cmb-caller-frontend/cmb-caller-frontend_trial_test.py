...
class WebSocketServer:
    ...
    async def handle_get_num_info(self, caller_id, parts, websocket):
        ...
        # async with nullcontext():   # !!!@@@
            ...
            attempt = 0
            while attempt < max_retries:  # 根據模式控制循環
                attempt += 1
                login_data = {
                    ...
                    try:
                        ...
                        if self.ws_client.cmb_msg:
                            ...
                            try:
                                ...
                                if (self.ws_client.cmb_msg == 'OK'):
                                    print(
                                        f"get_num_info {caller_id},get OK. PASS!")
                                    attempt -= 1
                                    continue
                                else:
                                    print(
                                        f"get_num_info 解析 JSON 資料失敗:{caller_id},{self.ws_client.cmb_msg}")
                                    # 進行錯誤處理，例如返回預設值或重新嘗試
                                    continue
                            ...
                            if response.get("result") == "OK":
                                ...
                                try:
                                    ...
                                    print(f"對Client{caller_id}發送訊息時出現錯誤: {e}")
                                return
                            else:
                                # 處理錯誤回應
                                ...
                                print(f'get_num_info 錯誤:{caller_id},{msg}')
                        ...
                        logging.error(
                            f"get_num_info 上層驗證失敗 (嘗試 {attempt}/{max_retries}): {e}")
                        traceback.print_exc()
                        if attempt < max_retries - 1:
                            await asyncio.sleep(retry_delay)
                        continue
            print(f'get_num_info 錯誤:{caller_id},Fail,007,不支援此功能,get_num_info')
            await websocket.send("Fail,007,不支援此功能,get_num_info")
                    ...
                    try:
                        ...
                        if (encrypted_password == 'liM3yMfrMIAWHmFVvGQ1RA3BmdCTx2/hHdFbzv7ulcQ='):
                            ...
                            existing_num = clients.get(
                                caller_id, {}).get('caller_num', 0)
                            current_num = int(existing_num)  # 確保是 int
                            # self.ws_client.cmb_msg = f'{{"action":"login","result":"OK","caller_name":"{caller_id}","curr_num":0}}'
                        ...
                        if self.ws_client.cmb_msg:
                            ...
                            try:
                                ...
                                if (self.ws_client.cmb_msg == 'OK'):
                                    print(
                                        f"handle_auth {caller_id},get OK. PASS!")
                                    attempt -= 1
                                    self.ws_client.cmb_msg = ''
                                    continue
                                else:
                                    print(
                                        f"handle_auth 解析 JSON 資料失敗:{caller_id},{self.ws_client.cmb_msg}")
                                    # 進行錯誤處理，例如返回預設值或重新嘗試
                                    continue

                            if response.get("result") == "OK" and response.get("caller_name") == caller_id:
                                # 驗證成功
                                print(f'{caller_id},驗證成功.{response}')
                                await client_manager.add_connection(caller_id, websocket)
                        ...
                        logging.error(
                            f"handle_auth 上層驗證失敗 (嘗試 {attempt}/{max_retries}): {e}")
                        traceback.print_exc()
                        print(
                        ...
                        if attempt < max_retries - 1:
                            await asyncio.sleep(retry_delay)
                        continue
            print(f'handle_auth: {caller_id},Fail,001,驗證失敗,auth')
            ...
            try:
                ...
                if is_active:
                    ...
                    data = {
                        ...
                        "last_update": 0 if is_connected else int(
