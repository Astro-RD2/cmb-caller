...
class WebSocketClient:
        ...
        while self.is_connected:
            try:
                async for message in self.ws:
                    print(
                        f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-4]} [收到訊息] 來源: WebSocket Server, 目的: {self.client_id}, 內容: {message}", flush=True)
                    msg_count += 1
                    # 追蹤最後一次 wifi_xxx 的回應
                    try:
    ...
    async def send_message_auth(self, message: str):
        ...
        try:
            ...
            print(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-4]} [發送訊息] 來源: {self.client_id}, 目的: WebSocket Server (認證), 內容: {message}")
        except websockets.exceptions.ConnectionClosed:
    ...
    async def send_message(self, message: str, is_response: bool = False):
        ...
        try:
            ...
            if is_response:
                print(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-4]} [回覆訊息] 來源: {self.client_id}, 目的: WebSocket Server, 內容: {message}")
            else:
                print(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-4]} [發送訊息] 來源: {self.client_id}, 目的: WebSocket Server, 內容: {message}")
        except websockets.exceptions.ConnectionClosed:
            print(f"[{self.client_id}] 發送時連接已關閉，嘗試重新連接...")
            self.is_connected = False
        except Exception as e:
...
async def wait_for_wifi_get_status(client: WebSocketClient, timeout=600):   # 60/0.1
    """
...
async def wifi_delete_profile_exec(client, delete_profile_cmd, device_id):
    """
...
async def simulate_wifi_operations(client: WebSocketClient, device_id: str):
    global msg_count
    ssid = "CMBz6666"
    password = "88888888"

    await asyncio.sleep(2)
    print(f"\n--- 開始模擬 {device_id} 的 WiFi 操作 ---")
    ...
    await wifi_delete_profile_exec(client, delete_profile_success2, device_id)
    ...
    await wifi_delete_profile_exec(client, delete_profile_fail_not_found, device_id)
    ...
    await wifi_delete_profile_exec(client, delete_profile_success, device_id)
    ...
    for i in range(10):
        ...
        await wifi_delete_profile_exec(client, delete_profile_payload, device_id)
