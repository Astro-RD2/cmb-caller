...
class WebSocketMonitor:
    def __init__(self, config: Dict[str, Any]):
        ...
        # self.reconnect_interval = 30  # 每30秒嘗試重連一次
        self.reconnect_interval = 5  # 每5秒嘗試重連一次

        self.continuous_reconnect_enabled = True

        # Cancellation flag to avoid spawning reconnects when shutting down
    ...
    async def _handle_continuous_reconnect(self, current_time: float):
        """處理持續重連"""
        print(f"🔴 [{datetime.now().strftime('%H:%M:%S')}] 執行持續重連嘗試 (#{self.reconnect_attempts + 1})")
    ...
    async def connect_and_login(self) -> bool:
        ...
        try:
            ...
            # 添加連線超時
            try:
                # 建議：保留 ping_interval=None（因使用自訂品質檢查），但設定 close_timeout
                self.ws = await asyncio.wait_for(
                    websockets.connect(
                        self.ws_url,
                        ping_interval=None,  # 禁用自動 ping
                        close_timeout=5  # 更短的 close timeout
                    ),
                    timeout=15
                )
            ...
            # 初次建立連線時設定 last_pong_time，避免品質檢查被誤判
    ...
    async def _websocket_listener(self):
        ...
        try:
            async for message in self.ws:
                ...
                # 添加訊息大小檢查
                if isinstance(message, (bytes, str)) and len(message) > 10 * 1024 * 1024:  # 10MB 限制
            ...
            logging.info("WebSocket 連線已關閉 (async for 結束)，監聽任務結束")
            
        except asyncio.CancelledError:
            logging.warning("WebSocket 監聽任務已被取消!")
            # 不做自動重連，讓上層 safe_close 處理
            return
        except websockets.exceptions.ConnectionClosedOK as e:
            logging.info(f"WebSocket 已正常關閉: code={e.code}, reason={e.reason}")
        except websockets.exceptions.ConnectionClosedError as e:
            logging.warning(f"WebSocket 非正常關閉: code={e.code}, reason={e.reason}")
            logging.debug(traceback.format_exc())
        except Exception as e:
    ...
    async def _cleanup_listener(self):
        ...
        try:
            ...
            try:
                ...
                logging.warning("訊息佇列已滿，無法放入終止訊號")
            except Exception:
                pass
        except:
    ...
    async def check_connection_quality_Q(self) -> Dict[str, Any]:
        """改進的連線品質檢查"""
        ...
        try:
            ...
            logging.debug(f"發送 ping 於 {ping_start}")
            await self.ws.ping()
            ...
            while time.time() - wait_start < self.quality_check_timeout:
                if (self.last_pong_time is not None and 
                    ...
                    logging.debug(f"收到 pong，回應時間: {response_time:.2f}ms")
                    return {
            ...
            logging.warning("內建 ping 未收到 pong 回應")
            
            # 方法2: 發送自訂 ping 訊息
            self.ping_sent_time = time.time()
            ping_msg = f"{self.client_id},ping"
            logging.debug(f"發送自訂 ping: {ping_msg}")
            ...
            while time.time() - wait_start < self.quality_check_timeout:
                if (self.last_pong_time is not None and 
                    ...
                    logging.debug(f"收到自訂 ping 的 pong，回應時間: {response_time:.2f}ms")
                    return {
                        "response_time_ms": round(response_time, 2),
                        "quality_ok": quality_ok,
                        "method": "custom_ping"
    ...
    async def check_connection_quality(self) -> Dict[str, Any]:
        ...
        修正：
        - 正確使用 websockets.ping() 返回的 waiter，並等待該 waiter 以確保收到對端的 pong frame。
        - 如果 builtin ping 無法在 timeout 內回應，會 fallback 到自訂 ping 訊息。
        """
        if self.connection_state != ConnectionState.CONNECTED or not self.ws:
            return {"response_time_ms": None, "quality_ok": False, "reason": "not_connected"}
        ...
        try:
            ...
            try:
                ...
                logging.warning(f"自訂 ping 發送失敗: {e}")
                self.ping_sent_time = None
    ...
    async def safe_close(self):
        ...
        if self.ws:
            try:
                # 嘗試正常關閉 handshake
                await asyncio.wait_for(self.ws.close(code=1000, reason="normal_close"), timeout=5)
            except asyncio.TimeoutError:
                logging.warning("關閉 WebSocket 時等待 close frame 超時，強制斷開")
                try:
                    # 強制取消連線物件
                ...
                logging.debug(f"關閉 WebSocket 發生例外: {e}")
            finally:
                self.ws = None
    ...
    async def attempt_reconnect(self) -> bool:
        ...
        if self.reconnect_attempts > 0:
            # 使用更緩慢的退避以避免狂重試
            delay = min(2 ** min(self.reconnect_attempts, 6), self.max_reconnect_delay)
