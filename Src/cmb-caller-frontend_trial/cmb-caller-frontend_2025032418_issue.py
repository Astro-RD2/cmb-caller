...
# @app.route('/periodic_send', methods=['GET', 'POST'])
def send_periodic_send():
    try:
        # logging.info(f"Minutely job event call {local_datetime()}")
        # print(f"Minutely job event call {local_datetime()}", flush=True)
        print("MS ", end='', flush=True)
        # 啟動後臺執行緒
        threadA = threading.Thread(target=periodic_send)
        threadA.start()
        # logging.info(f"Minutely job event return {local_datetime()}")
        # print(f"\nMinutely job event return {local_datetime()}", flush=True)
        print("ME ", end='\n', flush=True)
        return "Minutely job event return!"
    except Exception as e:
        logging.error(f"Error occurred during minutely job: {e}")
        print(f"Error occurred during minutely job: {e}", flush=True)
        print("MEF ", end='', flush=True)
        return f"Error occurred: {e}", 500  # 返回 HTTP 500 錯誤
...
async def periodic_send():
    global ws_server, platform_name
    """每分鐘發送現有之caller_id"""
    while True:
...
async def main():
    ...
    try:
        ...
        if platform_name != 'Cloud_Run':
            # 每分鐘發送現有之caller_id
            periodic_task = asyncio.create_task(periodic_send())
            # 保持主執行緒運行
        else:
            pass
            # port = int(os.environ.get('PORT', 8080))
