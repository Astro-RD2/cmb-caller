...
2025/04/09  Roy Ching    修正login後get不到目前的號碼
...
class WebSocketServer:
    ...
    async def on_message(self, caller_id, message, websocket, new_client):
        ...
        try:
            ...
            if (m_info != '0' and m_cmd == 'send') or (m_info != '0' and m_cmd == '') or (self.last_num == 0 and m_cmd == 'ping'):
                clients[caller_id]['messages'] = {
                    'timestamp': datetime.now(), 'message': client_msg}
                # logging.info(
                #     f"設定 clients[caller_id]['messages']:{datetime.now()},{client_msg}")
                # logging.info(
                #     f"clients[caller_id]:{clients[caller_id]}")
            ...
            if True:
                if websocket.open:
                    ...
                    #     continue
                    ...
                    elif (m_cmd == '' and m_info != '0') or m_cmd == 'send':      # SEND
                        ...
                        clients[caller_id]['caller_num'] = self.last_num
                        await websocket.send(f'OK,{caller_id},{self.last_num},send')
                    ...
                    # elif m_info == 0:
                        ...
                        # client_msg = clients[caller_id]['messages'][0] if clients[caller_id]['messages'] else ''
                        # if client_msg == '':
                        #     call_num = 0
                        # else:
                        #     call_num = extract_call_num(client_msg)
                        # await websocket.send(f'{caller_id},{call_num},{m_cmd}')
                        # if clients[caller_id]['messages']:
                        #     client_msg = clients[caller_id]['messages']['message']
                        #     call_num = extract_call_num(client_msg)
                        if clients[caller_id]['caller_num']:
                            call_num = clients[caller_id]['caller_num']
                        else:
                            logging.info(
                                f'get error: clients[caller_id]:{clients[caller_id]}')
                            call_num = 0
                        # logging.info(
                        #     f'get 回覆: OK,{caller_id},{call_num},{m_cmd}')
                        await websocket.send(f'OK,{caller_id},{call_num},{m_cmd}')
                    else:           # 連線資訊???
    ...
    async def handle_auth(self, caller_id, parts, websocket):
        """處理驗證請求 (使用 CMB Websocket Server 規格上傳至 CMB Websocket Server)"""
        if len(parts) != 3:
        ...
        for attempt in range(max_retries):
            ...
            if self.ws_client:
                try:
                    logging.info(f'第 {attempt + 1} 次嘗試: 發送登入資料')
                    await self.ws_client.send(json.dumps(login_data))

                    # 等待回應，設定超時10秒
                    start_time = time.time()
                    self.ws_client.cmb_msg = ''  # 清除訊息
                    ...
                    if self.ws_client.cmb_msg:
                        ...
                        # logging.info(f'收到上層回應:{response}')
                        if response.get("result") == "OK":
                            # 驗證成功，回應至client
                            # await websocket.send(f"OK,{response.get('caller_name','')},{response.get('curr_num','0')},auth")
                            ...
                            client_info = {
                                ...
                                'caller_num': clients[caller_id].get('caller_num', 0)
                            }
                            await client_manager.add_client(caller_id, client_info)
                            logging.info(f"客戶端 {caller_id} 驗證成功")
                            ...
                            if code == '003':
                                ...
                                msg = '007,文字錯誤/其它'
                            logging.info(f"客戶端 {caller_id} {msg}")
                    ...
                    tb = traceback.format_exc()
                    logging.error(
                        f"上層驗證失敗 (第 {attempt + 1}/{max_retries} 次嘗試): {e}\n{tb}")
        ...
        # 所有重試都失敗後的處理
        logging.error(f"上層驗證失敗，已達到最大重試次數 {max_retries}")
        await websocket.send(f"Fail,001,驗證失敗,auth")
        # await self.force_close_connection(websocket, caller_id, "上層驗證失敗")
    ...
    def parse_message(self, message):       # m_cmd 一律變為小寫
        ...
        try:
            ...
            if len(parts) == 3:
                if parts[1] == 'ping' or parts[1] == 'send' or parts[1] == 'auth':
