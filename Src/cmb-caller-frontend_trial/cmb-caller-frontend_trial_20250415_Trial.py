...
class ClientManager:
    def __init__(self):
        # 修改為存儲每個caller ID對應的多個連接
        # 結構: {caller_id: {'connections': set(), 'caller_num': int, 'caller_name': str}}
        self.clients = {}
            ...
            async for message in websocket:
                ...
                if caller_id not in clients:
                    if caller_id.startswith('a0') or caller_id.startswith('z'):
                        ...
                        message_x = f'{caller_id},AUTH,YV7X+xUEsMckopbXpp5sey+eosV8HYIGxa/fOS69/SU='
                        await self.handle_auth(caller_id, message_x.split(','), websocket)
                    else:
                ...
                if m_cmd == 'send' or (m_cmd == '' and m_info != '0'):
                    # 更新號碼並通知所有客戶端
                    new_num = int(m_info)
                    await client_manager.update_caller_info(caller_id, caller_num=new_num)
    ...
    while True:
        ...
        for caller_id, info in clients.items():
            try:
                ...
                # traceback.print_exc()
                start_time = datetime.now() - timedelta(seconds=(60-10))     # 出錯後將 start_time 設為xx秒前
                break   # 離開 for 迴圈

        print("", flush=True)
        # 記錄狀態
        Logger.log(
            f"總共有 {len(clients)} 個紀錄中Client, "
            f"{active_client} 個有效的Client, "
            f"{connected_client} 個連線中Client"
        )
