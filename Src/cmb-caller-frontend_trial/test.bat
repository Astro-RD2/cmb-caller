...
:: 檢查必要檔案是否存在
if not exist "CMB_frontend_test.py" (
    echo [錯誤] 找不到 CMB_frontend_test.py 檔案！
...
:: 顯示啟動資訊
echo [檢查] CMB_frontend_test.py ✓ 存在
...
for /L %%i in (1,1,%runCount%) do (
    ...
    :: 使用唯一的視窗標題啟動Python程序
    start "CMB_Instance_%%i" python CMB_frontend_test.py
    ...
    if not errorlevel 1 (
        echo [成功] 第 %%i 個執行個體已成功啟動 ✓
    ) else (
        echo [警告] 第 %%i 個執行個體啟動狀態未知
    )



    
    :: 除了最後一個執行個體，其他都要等待隨機間隔
    if %%i neq %runCount% (
	:: 產生 t1 到 t2 秒的隨機延遲
	set /a "delay=!random! %% range + t1"
	echo 隨機延遲: !delay! 秒
        timeout /t !delay! >nul
        echo.
    )
