...
void sendCallerNumber(unsigned long currentMillis) {
  ...
  if (nowStr != sendStr) {  // 沒連線一樣傳送至buffer
    // Serial.printf("sendCallerNumber nowStr:'%s', sendStr:'%s'\n", nowStr, sendStr);
    client_send(nowStr);
    // Serial.printf("sendCallerNumber set sendStr: '%s' -> '%s'\n", sendStr, nowStr);
    sendStr = nowStr;
    nowStrDemo = nowStr;
...
//     // 讀取回傳資料
//     String parts[4];  // 用於存儲分割後的字符串部分
//     int index = 0;
//     int start = 0;
//     int end = message.indexOf(',');
//     while (end != -1) {
//       parts[index++] = message.substring(start, end);
//       start = end + 1;
//       end = message.indexOf(',', start);
//     }
//     parts[index] = message.substring(start);  // 最後一部分
//     // 確保有4組資料且最後一組為update
//     if (index == 3 && parts[3] == "update") {
//       // int ValueStr = parts[2].toInt();
//       String ValueStr = parts[2];
//       if (nowStr != ValueStr) {
//         Serial.println("Extracted third value: " + ValueStr);
//         send_and_dis(ValueStr);
//       }
//     } else {
//       // Serial.println("Invalid format or data");
//     }
...
void send_and_dis(const String& message) {
...
// 時間記錄變數
unsigned long duringMs = 0;  // 通用計時變數
unsigned long testMs = 0;    // 測試功能計時

// LED狀態控制
byte led_light_en = 0;  // LED開關狀態標記(0:關,1:開)

// 通訊狀態標記
byte ble_flag = 0;  // BLE指令處理狀態
...
unsigned int testNum = 0;  // 測試用遞增編號
...
void UR2Recv() {
  ...
  if (mySerial.available()) {
    ...
    if (recMessage.length() == 9) {
      ...
      // if (recMessage.startsWith("BLE_") && recMessage.endsWith("_S")) {
      //       bleValue = recMessage.substring(4, recMessage.length() - 2);
      //       int bleNum = bleValue.toInt();
      //       Serial.println("Extracted number: " + String(bleNum));
      // } else {
...
void cldUR2Send(unsigned long num) {
  ...
  if (num != 0 && num < 1000) {
    ...
    Serial.println(sendCldStr);    // 序列埠輸出用於調試
 ...
 */
// void testUR2() {
//   if ((millis() - testMs) >= 15000) {  // 15秒間隔檢查
//     testMs = millis();                 // 重置計時器
//     testNum++;                         // 測試編號遞增
//     cldUR2Send(testNum);               // 發送CLD指令

//     // // LED狀態切換
//     // digitalWrite(BLED, !led_light_en);  // 反轉LED狀態
//     // led_light_en = !led_light_en;       // 更新狀態標記
//   }
// }
...
// // 初始化設定
// void setup() {
//   Serial.begin(115200);  // 初始化序列埠(調試用)

//   pinMode(BLED, OUTPUT);    // 設定藍色LED為輸出模式
//   digitalWrite(BLED, LOW);  // 初始狀態關閉

//   // 初始化UART2
//   mySerial.begin(9600, SERIAL_8N1, RX2, TX2);  // 波特率9600,8資料位,無校驗,1停止位
//   mySerial.setTimeout(200);                    // 設置讀取超時200ms
//   Serial.println("UART Setup OK");             // 輸出初始化完成訊息
// }
